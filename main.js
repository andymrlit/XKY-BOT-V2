   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +50934767877")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number 😍\nFor example: +50934767877 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
...
try{
		...
		if (update.connection == "open" || update.receivedPendingNotifications == "true") {
	        ...
	        console.log(color(`\n${themeemoji} YT CHANNEL: mrlit`,'magenta'))
            console.log(color(`${themeemoji} GITHUB: andymrlit `,'magenta'))
            console.log(color(`${themeemoji} INSTAGRAM: @andy_mr_lit `,'magenta'))
...
"sourceUrl": `${wagc}`}}})
                	...
                	const xeonbuffer = await getBuffer(ppuser)
                    const xeontime = moment.tz('America/Port-au-Prince').format('HH:mm:ss')
	                const xeondate = moment.tz('America/Port-au-Prince').format('DD/MM/YYYY')
                	let xeonName = num
